...
export default function DashboardPage({ setTab }) {
  ...
  useEffect(() => {
    async function fetchAll() {
      const [e, q, r] = await Promise.all([
        ...
        supabase.from('supplier_responses').select('*').order('created_at', { ascending: false }),
      ])
      setEnquiries(e.data || [])
      setQuotes(q.data || [])
      setResponses(r.data || [])
  ...
  const totalMargin  = quotes.reduce((sum, q) => sum + (q.quoted_price - q.supplier_price) * 1, 0)
  const totalRevenue = quotes.reduce((sum, q) => sum + q.quoted_price * 1, 0)
  ...
  const activeEnquiries = enquiries.filter(e => e.status !== 'closed')
  ...
  return (
    <div style={{ padding: '28px 32px', background: bg, minHeight: '100vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, marginBottom: 24 }}>
        <StatCard label="Total Revenue" value={`₹${totalRevenue.toLocaleString()}`} color="#6EC89E" sub="from quoted orders" />
        <StatCard label="Total Margin"  value={`₹${totalMargin.toLocaleString()}`}  color={C.accent} sub="your profit" />
      </div>
      ...
      <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, overflow: 'hidden' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>{['#', 'Date', 'Buyer', 'Paper Type', 'GSM', 'Coating', 'Shade', 'Qty', 'Supplier Rate', 'Buyer Rate', 'Responses', 'Status'].map(h => <th key={h} style={th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {activeEnquiries.length === 0 && (
                <tr><td colSpan={12} style={{ ...td, textAlign: 'center', color: muted }}>No active enquiries</td></tr>
              )}
              {activeEnquiries.slice(0, 10).map((e, i) => {
                const sc = { open: '#6E9EC8', responded: '#C8A96E', quoted: '#6EC89E', closed: '#666' }[e.status] || '#666'
                const quote = quotes.find(q => q.enquiry_id === e.id)
                const respCount = responses.filter(r => r.enquiry_id === e.id).length
                return (
                  <tr key={e.id}
                  ...
                  >
                    <td style={{ ...td, color: muted, fontSize: 11 }}>{String(i+1).padStart(2,'0')}</td>
                    ...
                    <td style={{ ...td, color: muted }}>{e.coating}</td>
                    <td style={{ ...td, color: muted }}>{e.shade}</td>
                    <td style={{ ...td, whiteSpace: 'nowrap' }}>{e.quantity?.toLocaleString()} {e.unit}</td>
                    <td style={td}>{quote ? <span style={{ color: C.green, fontWeight: 600 }}>₹{quote.supplier_price}/kg</span> : <span style={{ color: muted }}>—</span>}</td>
                    <td style={td}>{quote ? <span style={{ color: C.accent, fontWeight: 600 }}>₹{quote.quoted_price}/kg</span> : <span style={{ color: muted }}>—</span>}</td>
                    <td style={{ ...td, textAlign: 'center' }}>
                      <span style={{ background: respCount > 0 ? C.green+'22' : C.muted+'22', color: respCount > 0 ? C.green : C.muted, borderRadius: 4, padding: '2px 8px', fontSize: 11 }}>{respCount}</span>
                    </td>
                    <td style={td}>
